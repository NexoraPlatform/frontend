...
function TrustFlowScene() {
  ...
  function createConnections() {
    ...
    // Create tubes for active connections
  ...
  function activateConnection(conn: Connection) {
    ...
    // Create lock mesh (simplified 3D lock)
    ...
    for (let i = 0; i < numPayloads; i++) {
      const availableMesh = payloadPoolRef.current.find(m => !m.visible);
      if (availableMesh) {
  ...
  useFrame((state, delta) => {
    ...
    connections.forEach((conn) => {
      ...
      // Update curve based on node positions
    ...
    if (timeRef.current > nextConnectionChangeRef.current) {
      ...
      // Deactivate 2-3 random active connections
      ...
      // Activate 2-3 random inactive connections
