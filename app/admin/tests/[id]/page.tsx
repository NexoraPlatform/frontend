...
export default function TestDetailsPage({ params }: TestDetailsPageProps) {
  ...
  const { data: test, loading, error, refetch } = useTest(params.id);
    ...
    try {
      if (action === 'delete') {
        if (confirm('Ești sigur că vrei să ștergi acest test?')) {
          await apiClient.deleteTest(params.id);
          router.push('/admin/tests');
        }
      } else if (action === 'activate') {
        await apiClient.updateTestStatus(params.id, 'ACTIVE');
        refetch();
      } else if (action === 'deactivate') {
        await apiClient.updateTestStatus(params.id, 'INACTIVE');
        refetch();
      }
    } catch (error: any) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Link href={`/admin/tests/${params.id}/statistics`}>
        ...
        <Button
          variant="outline"
          onClick={() => window.open(`/tests/preview/${params.id}`, '_blank')}
        >
