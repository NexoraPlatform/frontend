...
interface QuestionStat {
  questionId: string;
  type: string; // sau un union literal dacă ai definit tipurile
  points: number;
  question: string;
  successRate: number;
  avgTimeSpent: number;
...
export default function TestStatisticsPage({ params }: TestStatisticsPageProps) {
  const router = useRouter();
  const { data: test, loading: testLoading, error: testError } = useTest(params.id);
  const { data: stats, loading: statsLoading, error: statsError } = useTestStatistics(params.id);
