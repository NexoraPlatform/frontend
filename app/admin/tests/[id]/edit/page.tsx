...
export default function EditTestPage({ params }: EditTestPageProps) {
  ...
  const { data: testData, loading: testLoading, error: testError } = useTest(params.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.updateTest(params.id, {
        ...formData,
        questions
      });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center space-x-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold">Editează Test</h1>
          <p className="text-muted-foreground">
            Modifică detaliile și întrebările testului
          </p>
        </div>
      </div>
